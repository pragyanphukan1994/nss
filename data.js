...
function init({ sheetCsv, containerId, filters, searchField, cardFields, extraFields, detailLink }) {
  fetchCsv(sheetCsv).then(all => {
    ...
    all.forEach(r => {
      if (filters.includes("Department")) deptSet.add(r["Department"]);
      if (filters.includes("Year of Admission")) yearSet.add(r["Year of Admission"]);
    });
    ...
    function cardHtml(r) {
      ...
      return `
        <div class="card" onclick="location='${detailLink + encodeURIComponent(r['Roll No'])}'">
          <img src="${photo}" alt="${r['Student Name']}">
...
async function loadProfile(sheetCsv, keyField, keyValue, containerId) {
  ...
  const photo = getPhotoUrl(rec["Passport Photo"]);
  const profileHTML = `
    <div class="card" style="width:400px;margin:auto;">
      <img src="${photo}" alt="${rec['Student Name']}" style="width:120px;height:120px;margin-bottom:10px;">
      <h2>${rec["Student Name"]}</h2>
      ${Object.entries(rec).map(([k, v]) => `<p><strong>${k}:</strong> ${v}</p>`).join("")}
    </div>
  `;
